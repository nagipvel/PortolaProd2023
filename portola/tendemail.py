...
from portola.serializers import ForgotPasswordSerializer
...
class TendEmail(BaseCommand):
    ...
    def handle(self, *args, **options):
        # restart the clock every time we get called:
        self.last_month = datetime.today() - timedelta(days=self.MONTH)
        self.last_week = datetime.today() - timedelta(days=self.WEEK)
        ...
        if (self.epoch - date.today()).days % 7 == 0: # do the sunday night stuff
        ...
        for entity in Entity.objects.all():
            ...
            for profile in entity.profile_set.all():
                ...
                for project in profile.user.project_set.all():
                    ...
                    count = documents.filter(created__gte = self.yesterday).count()
                    if count == 0: continue
                    # documents = documents.filter(issued_date__lte = self.last_week)
                    for document in documents:
                        days_old = datetime.today().date() - document.created.date()
                        email_rows.append({
                        'number':document.id,
                        'title':document.title,
